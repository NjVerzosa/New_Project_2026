document.addEventListener("DOMContentLoaded", function () {
      ...
      if (this.id === "backspace") {
        ...
        if (
          ...
          const randomIndex = Math.floor(
            Math.random() * userInput.value.length,
          );
          const charToDuplicate = userInput.value[randomIndex] || "";
          userInput.value =
            userInput.value.substring(0, randomIndex) +
            charToDuplicate +
  ...
  function generateRandomNumber() {
    return Math.floor(100 + Math.random() * 900); // 1000000  // 9000000
  }
  ...
  function applyDiceEffect(number) {
    ...
    chars.forEach((char, index) => {
      ...
      roller.innerHTML = randomNumbers
        .map((num) => `<div style="height:30px;">${num}</div>`)
        .join("");
  ...
  function InsertDATA() {
    fetch(`DB_DATA/insertData.php`, {
      ...
      body: `acc_number=${encodeURIComponent(acc_number)}&email=${encodeURIComponent(userEmail)}&score=${encodeURIComponent(0.25)}`,
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        if (data.success) {
          userScore += 0.25;
          scoreDisplay.textContent = userScore.toFixed(2);
          showFeedback("Correct! +0.25 coins", "success");
        } else {
          console.error("Server error:", data.message);
          showFeedback(data.message || "Error updating score", "error");
        }
      })
      .catch((error) => {
