...
try {
    ...
    $updateUserStmt = $con->prepare("UPDATE users SET earned_coins = earned_coins + 0.10 WHERE acc_number = :acc_number AND email = :email");
    ...
    $stmt = $con->prepare("SELECT id, score, date FROM riddle_game WHERE acc_number = :acc_number AND email = :email AND date = :date FOR UPDATE");
    $stmt->execute([
        'acc_number' => $accNumber,
        'email' => $email,
        'date' => $date
    ...
    if ($progress) {
        $updateStmt = $con->prepare("UPDATE riddle_game SET score = score + :score WHERE id = :id");
        $updateStmt->execute([
            'score' => $score,
            'id' => $progress['id']
        ]);
        ...
        if ($updateStmt->rowCount() === 0) {
            throw new Exception("No records updated in riddle_game");
        }
    } else {
        $insertStmt = $con->prepare("INSERT INTO riddle_game (acc_number, email, date, score) VALUES (:acc_number, :email, :date, :score)");
        $insertStmt->execute([
            'acc_number' => $accNumber,
            'email' => $email,
            'date' => $date,
