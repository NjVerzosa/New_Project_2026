...
include __DIR__ . '/includes/config.php';  // If database.php is one level up in config folder

/**
 * Check if the request is from a known bot or crawler.
 *
 * @return string|false Returns the bot name if detected, otherwise false.
 */
...
if (!isset($_SESSION["acc_number"]) || $_SESSION['role'] === "Admin") {
...
$stmt = $con->prepare("SELECT * FROM users WHERE acc_number = ?");
